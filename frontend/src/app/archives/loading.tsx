      ...
      <div className="md:pt-6">
        ...
        >
          <main className="relative w-full bg-wechat-white pb-8 md:pb-12">
            {/* ProfileTopBar 骨架：fixed 与真实结构一致 */}
            <header
              data-topbar
            ...
            <div className="px-4 py-2 sm:px-5 md:px-6">
              ...
              <div className="flex flex-wrap gap-2">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div
                    key={i}
                    className={`aspect-square w-[72px] shrink-0 rounded sm:w-[80px] ${BLOCK}`}
                  />
                ))}
              </div>
            </div>

            {/* 时间线骨架：左侧日期 + 右侧动态卡片（精简版，无头像） */}
            <div className="mt-2">
              {Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="flex">
                  ...
                  <div className="w-[68px] shrink-0 pl-4 pt-2 sm:pl-5 md:pl-6">
                    {i === 0 && (
                      <div className="flex items-baseline gap-0.5">
                        <div className={`h-5 w-5 rounded ${BLOCK}`} />
                  ...
                  <div className="min-w-0 flex-1 px-2 py-2">
                    <div className="space-y-2">
                      ...
                      <div className={`mt-2 h-24 w-full rounded-[4px] ${BLOCK}`} />
