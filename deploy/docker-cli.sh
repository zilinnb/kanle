...
echo -e "  ${GREEN}✨ 预构建镜像通用，任何 IP/域名都能用！${NC}"
echo ""
...
# ============================================================
echo ""
echo -e "${CYAN}${BOLD}[2/5] 前端端口配置${NC}"
prompt FRONTEND_PORT "前端访问端口 [3000]: " "$DEFAULT_FRONTEND_PORT"
echo -e "  ${GREEN}→ 后端不对外暴露端口（通过 Next.js rewrites 代理，只需放行 ${FRONTEND_PORT}）${NC}"
...
echo -e "  后端端口:       ${GREEN}不对外暴露${NC}（Docker 内部通信）"
...
echo -e "  ${BOLD}----------------------------------------${NC}"
echo -e "  ${GREEN}只需放行端口 ${FRONTEND_PORT}，无需放行 4000！${NC}"
echo ""
...
# Backend — 不映射 4000 端口，CLIENT_URL 用 Docker 内部地址（用于 revalidate 回调）
if docker ps -a --format '{{.Names}}' | grep -q "^${BACKEND_CONTAINER}$"; then
  ...
  docker run -d \
    ...
    -e CLIENT_URL="http://${FRONTEND_CONTAINER}:3000" \
  ...
  echo -e "${GREEN}[3/4]${NC} 后端已启动（不对外暴露端口，通过 rewrites 代理）"
fi

# Frontend — 用预构建镜像（NEXT_PUBLIC_API_URL=/api 通用，BACKEND_URL 在构建时已内联）
if docker ps -a --format '{{.Names}}' | grep -q "^${FRONTEND_CONTAINER}$"; then
  ...
  echo -e "${GREEN}[4/4]${NC} 前端已启动 ($FRONTEND_IMAGE)"
fi
